# Agent Guidelines for Foundatio.AzureStorage
...
```bash
# Build
dotnet build Foundatio.AzureStorage.slnx

# Test
dotnet test Foundatio.AzureStorage.slnx

# Format code
dotnet format Foundatio.AzureStorage.slnx
```
...
```text
src
└── Foundatio.AzureStorage                    # Azure provider implementations
    ├── Storage                      # IFileStorage implementations for Azure
    ├── Queues                       # IQueue<T> implementations for Azure
    └── Extensions                   # Extension methods for DI
tests
└── Foundatio.AzureStorage.Tests             # Unit and integration tests
```
...
1. **Builds successfully**: `dotnet build Foundatio.AzureStorage.slnx` exits with code 0
2. **All tests pass**: `dotnet test Foundatio.AzureStorage.slnx` shows no failures
...
- Mirror the main code structure (e.g., `Storage/` tests for `src/Foundatio.AzureStorage/Storage/`)
...
# Specific test file
dotnet test --filter "FullyQualifiedName~AzureStorageTests"
