# Copilot AI Coding Agent Instructions for Foundatio.AzureStorage
...
- `src/Foundatio.AzureStorage/` — Main library code for AzureStorage integration.
- `tests/Foundatio.AzureStorage.Tests/` — Unit and integration tests for AzureStorage features.
- `build/` — Shared build props, strong naming key, and assets.
- `Foundatio.AzureStorage.slnx` — Solution file for development.

## Developer Workflows
- **Build:** Use the VS Code task `build` or run `dotnet build` at the repo root.
- **Test:** Use the VS Code task `test` or run `dotnet test tests/Foundatio.AzureStorage.Tests`.
